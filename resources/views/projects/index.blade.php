                        ...
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col-md-12 mt-3">
                                    ...
                                    <div id="sort-rules-container">
                                        <!-- Sort rules will be added here -->
                                    </div>
                                    <button type="button" class="btn btn-secondary btn-sm" id="add-sort-rule">
...
@push('scripts')
    ...
    $(document).ready(function () {
        ...
        function addSortRuleRow(column = '', direction = 'asc') {
            let options = '<option value="">@lang("app.select") @lang("app.column")</option>';
            for (const [key, value] of Object.entries(sortableColumns)) {
                options += `<option value="${key}" ${key === column ? 'selected' : ''}>${value}</option>`;
            }

            const html = `
                <div class="row mb-2 sort-rule-row">
                    <div class="col-md-5">
                    ...
                    <div class="col-md-2">
                        <button type="button" class="btn btn-secondary btn-sm remove-sort-rule" style="margin-top: 5px;">
                            <i class="fa fa-times"></i>
                        </button>
                    </div>
                </div>
            `;
            $('#sort-rules-container').append(html);
            $('#sort-rules-container .select-picker').selectpicker();
        }

        $('#add-sort-rule').click(function() {
            addSortRuleRow();
        });

        $(document).on('click', '.remove-sort-rule', function() {
            $(this).closest('.sort-rule-row').remove();
        });

         $('#clear').click(function() {
            $('#sort-rules-container').empty();
