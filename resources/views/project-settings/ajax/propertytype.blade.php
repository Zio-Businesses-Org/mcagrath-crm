<div class="col-lg-12 col-md-12 ntfcn-tab-content-left w-100 p-4">
    ...
    <div class="table-responsive">
        <x-table class="table-bordered">
            <x-slot name="thead">
                <th>#</th>
                <th width="35%">@lang('Property Type Name')</th>
                <th class="text-right">@lang('app.action')</th>
            </x-slot>

            @forelse($propertyType as $key => $category)
                    ...
                    <td class="text-right">
                        <div class="task_view">
                            <a href="javascript:;" data-category-id="{{ $category->id }}"
                               class="editProperty task_view_more d-flex align-items-center justify-content-center">
                                <i class="fa fa-edit icons mr-1"></i> @lang('app.edit')
                            </a>
                        </div>
                        <div class="task_view mt-1 mt-lg-0 mt-md-0 ml-1">
                            <a href="javascript:;" data-category-id="{{ $category->id }}"
                               class="delete-property task_view_more d-flex align-items-center justify-content-center">
...
<script>
    ...
    $('#addPropertyType').click(function () {
        var url = "{{ route('project-settings.createPropertyType') }}";
    ...
    $('.editProperty').click(function () {
        ...
        var url = "{{ route('propertyType.edit', ':id') }}";
    ...
    $('body').on('click', '.delete-property', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                var url = "{{ route('propertyType.destroy', ':id') }}";
                url = url.replace(':id', id);
